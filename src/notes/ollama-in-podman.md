...
- date: 2026-06-29T23:16:25+03:00
  change: Добавил команду для получения групп `video` и `render`.
---
...
Да, образ толстенький. Кстати, пока не забыл, ROCm требует, чтобы пользователь
находился в группах `video` и `render`, так что сразу добавляемся.

```shell-session
$ sudo usermod -aG video,render $USER
```

Теперь надо запустить образ. На странице Docker Hub есть команда для запуска на
AMD GPU. Немного её модифицируем, добавив права опцией `container_runtime_t` и
сохраняя пользовательские группы опцией `keep-groups`. Это нужно для Podman,
ведь он выдаёт контейнерам меньше прав по-умолчанию. Кроме этого добавил
переменную окружения `OLLAMA_KEEP_ALIVE`, чтобы модели автоматически
выключались после 30 минут бездействия, а не 5, как обычно.
